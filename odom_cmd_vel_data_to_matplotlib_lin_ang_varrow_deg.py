...
def quaternion_to_yaw(qx, qy, qz, qw):
    # Nur Yaw-Winkel berechnen (Rotation um Z-Achse)
...
for i, is_active in enumerate(active_mask):
    if is_active and current_start is None:
        current_start = cmd_timestamps[i] - odom_timestamps.min()
    elif not is_active and current_start is not None:
        active_periods.append((current_start, cmd_timestamps[i] - odom_timestamps.min()))
        current_start = None
if current_start is not None:
    active_periods.append((current_start, cmd_timestamps[-1] - odom_timestamps.min()))
...
arrow_length = 0.1  # Länge der Pfeile in Metern
text_offset = 0.05  # Abstand des Textes vom Pfeil
...
ax_lin.plot(cmd_timestamps - odom_timestamps.min(), cmd_linear_vel, 
            color='darkgreen', 
            linewidth=1.5,
            linestyle='--',
            alpha=0.7,
...
ax_ang.plot(cmd_timestamps - odom_timestamps.min(), cmd_angular_vel, 
            color='purple', 
            linewidth=1.5,
            linestyle='--',
            alpha=0.7,
...
plt.savefig('roboter_trajektorie_mit_ausrichtung.png', dpi=300, bbox_inches='tight')
plt.show()
